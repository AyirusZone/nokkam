"""nokkam entrypoint. Wires config -> db -> repositories -> services -> UI."""
...
from nokkam.config import AppConfig, load_config
...
from nokkam.infra.notification_adapter import send_notification
...
class NokkamApp(App):
    ...
    def action_toggle_theme(self) -> None:
        self.theme = "nokkam-light" if self.theme == "nokkam-dark" else "nokkam-dark"

    def notify_desktop(self, message: str, title: str = "nokkam") -> None:
        send_notification(message, title)

    def _check_reminders(self) -> None:
        for task in self.reminder_service.due_soon():
            self.notify(f"Due soon: {task.title}")
            self.notify_desktop(f"Due soon: {task.title}", title="nokkam reminder")
    ...
    def action_open_pomodoro(self) -> None:
        self._goto_home()
        from nokkam.presentation.screens.pomodoro_screen import PomodoroScreen
